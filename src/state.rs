use nom::{le_u32, Err, HexDisplay, Offset};
use parser::{self, block, header, strf, AVIStreamHeader, BitmapInfoHeader, Block, FccType};
use std::cmp;
...
pub fn parse_video_index_stream(
    ...
    match state {
        VideoIndexState::Initial(header) => match strf(input) {
            Err(Err::Error(_)) => (0, VideoIndexState::Error),
        ...
        VideoIndexState::BMP(header, bmp_header) => match tuple!(input, tag!(b"JUNK"), le_u32) {
            Err(Err::Error(_)) => (0, VideoIndexState::Error),
